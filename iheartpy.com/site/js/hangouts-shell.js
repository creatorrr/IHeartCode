...
var _START=0;
var participants = [];
var hangoutState = {'id':'none','action':''};
var hangoutLocalUser = gapi.hangout.getParticipantById(gapi.hangout.getParticipantId());//Got it!
var hangoutId = hangoutLocalUser.hangoutId;
...
function trim(str){
	...
	var shellClient = {}
		...
		shellClient.printResult = function(result, user = hangoutLocalUser) {
				result='\n\n>>> '+result;
				shellEditor.save();
...
function start(){
		var message = "";
		gapi.hangout.removeAppVisibleListener(start);
		displayNotice(message);
		gapi.hangout.layout.setChatPaneVisible(false);
		}

function participantInit(){
		participants = gapi.hangout.getParticipants();
		}
		
function stateInit(){
		hangoutState = gapi.hangout.data.getStateMetadata();
		var user = gapi.hangout.getParticipantById(hangoutState.id);
		var result = hangoutState.action;
		shellClient.printResult(result, user);
		}


/*START- Really?*/

gapi.hangout.addAppVisibleListener(shellInit);
gapi.hangout.addApiReadyListener(start);
gapi.hangout.addParticipantAddedListener(participantInit);
gapi.hangout.addParticipantRemovedListener(participantInit);
gapi.hangout.data.addStateChangeListener(stateInit);
